...
{
if (!madlazh)
	madlazh = new Tmadlazh(this);
  madlazh->Show();
  madlazh->BringToFront();
DM1->tajwid->Close();
DM1->tajwid->SQL->Clear();
DM1->tajwid->SQL->Add("select *from hukumtajwid where namahukum='"+mharfi->Caption+"'");
DM1->tajwid->Open();
madlazh->Memo1->Lines->Text=DM1->tajwid->FieldByName("penjelasan")->AsString;
}
//---------------------------------------------------------------------------
...
void __fastcall Tk2::abMouseLeave(TObject *Sender)
{
	ab->Color=0x003F3122;
...
void __fastcall Tk2::abMouseMove(TObject *Sender, TShiftState Shift, int X, int Y)
...
void __fastcall Tk2::abClick(TObject *Sender)
{
  if (!mhuruf)
	mhuruf = new Tmhuruf(this);
  mhuruf->Show();
