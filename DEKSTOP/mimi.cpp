...
void __fastcall Tmi_mi::FormShow(TObject *Sender)
{
DM1->tajwid->Close();
DM1->tajwid->SQL->Clear();
DM1->tajwid->SQL->Add("select *from hukumtajwid where id='19'");
DM1->tajwid->Open();
...
void __fastcall Tmi_mi::abMouseLeave(TObject *Sender)
{
ab->Color=clWhite;
...
void __fastcall Tmi_mi::abMouseMove(TObject *Sender, TShiftState Shift, int X, int Y)
...
void __fastcall Tmi_mi::abClick(TObject *Sender)
{
  if (!mhuruf)
	mhuruf = new Tmhuruf(this);
  mhuruf->Show();
